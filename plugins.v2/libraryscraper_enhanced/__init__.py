...
class LibraryScraperEnhanced(_PluginBase):
    # 插件名称
    plugin_name = "媒体库刮削增强版"
    # 插件描述
    plugin_desc = "定时对媒体库进行刮削，补齐缺失元数据和图片。支持多线程并行刮削，大幅提升速度。"
    # 插件图标
    plugin_icon = "scraper.png"
    # 插件版本
    plugin_version = "2.1.3-enhanced"
    # 插件作者
    plugin_author = "jxxghp + jyh"
    # 作者主页
    author_url = "https://github.com/jxxghp"
    # 插件配置项ID前缀
    plugin_config_prefix = "libraryscraper_enhanced_"
    # 加载顺序
    plugin_order = 8
    # 可使用的用户级别
    user_level = 1
    ...
    def init_plugin(self, config: dict = None):
        ...
        if config:
            ...
            self._scraper_threads = int(config.get("scraper_threads", 4))
        ...
        if self._enabled or self._onlyonce:
            ...
            if self._onlyonce:
                ...
                self.update_config({
                    ...
                    "scraper_threads": self._scraper_threads
                })
                                ...
                                'content': [
                                    {
                                        ...
                                        'props': {
                                            'model': 'scraper_threads',
                                            'label': '刮削线程数',
                                            'type': 'number',
                                            'min': 1,
                                            'max': 20,
                                            'hint': '多线程刮削可提高速度，建议4~8，过高可能被TMDB限流'
                                        }
                                    }
                                ]
                            }
    ...
    def __libraryscraper(self):
        ...
        # 开始刮削（多线程）
        if scraper_paths:
            logger.info(f"开始刮削，共 {len(scraper_paths)} 个目标，线程数：{self._scraper_threads}")
            with ThreadPoolExecutor(max_workers=self._scraper_threads) as executor:
                future_map = {
                    executor.submit(self.__scrape_path,
                                    path=item[0], mtype=item[1],
                                    target_type=item[2], tmdbid=item[3]): item[0]
                ...
                for future in as_completed(future_map):
                    ...
                    try:
                        ...
                        logger.error(f"刮削失败：{future_map[future]} - {str(e)}")
        else:
    ...
    def __get_scrape_item(
        ...
        if rename_format_level >= 1:
            ...
            if len(relative_parts) > rename_format_level:
                ...
                return media_path, mtype, LibraryScraperEnhanced._target_dir, tmdbid

        # 扁平目录或自定义重命名格式无目录层级时，退回到单文件刮削，避免分类目录识别失败。
        return file_path, mtype, LibraryScraperEnhanced._target_file, tmdbid
